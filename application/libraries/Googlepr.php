...
class Googlepr
{
    ...
    public function GetPR($url, $forceNoCache = false)
    {
    ...
    public function debugRes($what, $sowhat)
    {
        if ($this->debug == true) {
    ...
    public function readCacheResult($url)
    {
        ...
        $path_ = (isset($urlp['query']) and strlen($urlp["query"]) > 0) ? urlencode($urlp["path"] . $urlp["query"]) : "default";
    ...
    public function updateCacheResult($url, $content)
    {
        if ($this->useCache != true) {
            return false;
        }
        ...
        if (!file_exists($cache_file)) {
            ...
            if (!@touch($cache_file)) {
                $this->debugRes("error", "unable to create cache file");
            }
            if (!@chmod($cache_file, 0777)) {
                $this->debugRes("error", "unable to chmod cache file");
            }
        }
    ...
    public function get_pr($url)
    {
        ...
        if ($fp) {
            $out = "GET /tbr?client=navclient-auto&ch=$ch";
