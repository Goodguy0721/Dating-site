...
class Auth_OpenID_FileStore extends Auth_OpenID_OpenIDStore
{
    ...
    public function cleanupNonces()
    {
        ...
        $nonces = self::_listdir($this->nonce_dir);
        $now = time();
        ...
        foreach ($nonces as $nonce_fname) {
            ...
            if (abs($timestamp - $now) > $Auth_OpenID_SKEW) {
                self::_removeIfPresent($nonce_fname);
                $removed += 1;
            }
        }
    ...
    public function getAssociationFilename($server_url, $handle)
    {
        ...
        $parts = explode('/', $rest);
        $domain = self::_filenameEscape($parts[0]);
        $url_hash = self::_safe64($server_url);
        if ($handle) {
            $handle_hash = self::_safe64($handle);
        } else {
            $handle_hash = '';
        }
    ...
    public function getAssociation($server_url, $handle = null)
    {
        ...
        if ($handle) {
            ...
            $association_files =
                self::_listdir($this->association_dir);
    ...
    public function removeAssociation($server_url, $handle)
    {
        ...
        if ($assoc === null) {
            ...
            return self::_removeIfPresent($filename);
        }
    }
    ...
    public function clean()
    {
        ...
        foreach ($this->_allAssocs() as $pair) {
            ...
            if ($assoc->getExpiresIn() == 0) {
                self::_removeIfPresent($assoc_filename);
            }
        }
    }
    ...
    /**
     */
    public static function _mkdtemp($dir)
    {
        foreach (range(0, 4) as $i) {
