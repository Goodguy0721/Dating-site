...
class Auth_OpenID
{
    ...
    public static function getQuery($query_str = null)
    {
        ...
        if ($query_str !== null) {
            $data = self::params_from_string($query_str);
          ...
          // data is available.
          $data = self::params_from_string($_SERVER['QUERY_STRING']);
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if ($str === false) {
                    ...
                    $post = self::params_from_string($str);
                }
    ...
    public static function appendArgs($url, $args)
    {
        ...
        return $url . $sep . self::httpBuildQuery($args);
    }
     ...
     */
    public static function urlunparse($scheme, $host, $port = null, $path = '/',
                        $query = '', $fragment = '')
    {
        if (!$scheme) {
            $scheme = 'http';
    ...
    public static function normalizeUrl($url)
    {
        ...
        list($defragged, $frag) = self::urldefrag($normalized);
