...
class Auth_Yadis_Yadis
{
   ...
   /**
    */
   public static function _getHeader($header_list, $names)
   {
    ...
    /**
     */
    public static function _getContentType($content_type_header)
    {
                      ...
                      $extra_ns_map = null, $timeout = 20)
    {
        ...
        $result->content_type = self::_getHeader(
                                       $response->headers,
                                       array('content-type'));

        if ($result->content_type &&
            (self::_getContentType($result->content_type) ==
             Auth_Yadis_CONTENT_TYPE)) {
            $result->xrds_uri = $result->normalized_uri;
        } else {
            $yadis_location = self::_getHeader(
                                                 $response->headers,
