...
//Charset maps
define("MAP_DIR", dirname(__FILE__) . '/map');
...
class utf8
{
  ...
  //Constructor
  public function __construct($charset = CP1251)
  {
      $this->loadCharset($charset);
  }

  //Load charset
  public function loadCharset($charset)
  {
  ...
  //Error handler
  public function onError($err_code, $err_text)
  {
  ...
  public function _utf8ToChar(&$chars, &$idx)
  {
      ...
      } elseif (($chars[$idx] >= 224) && ($chars[$idx] <= 239)) { // 3 bytes
      $utf =    (intval($chars[$idx] - 224)   << 12) +
                (intval($chars[++$idx] - 128) << 6) +
                (intval($chars[++$idx] - 128) << 0);
      } elseif (($chars[$idx] >= 192) && ($chars[$idx] <= 223)) { // 2 bytes
