function adminPolls(optionArr) {
	...
	this.bind_events = function(){
		$('#use_expiration').bind('click', function() {
			if (this.checked){
...
function adminPollsAnswers(optionArr) {
	...
	this.bind_events = function() {
		$('input.default_answer').off().on('keyup change click mouseup', function() {
			_self.update_preview();
		});
                $('input.color-pick').off().on('change', function(){
                    _self.update_preview();
                });
		$('a.delete_answer').off().on('click', function() {
			var id = +$(this).attr('id').replace(/\D+/g, '');
			_self.delete_answer(id);
		});
		
		$('#add_answer').off().on('click', function(){
			_self.add_answer();
		});
	}
