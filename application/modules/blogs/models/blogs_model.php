...
if (!defined('BLOGS_TABLE'))
	define('BLOGS_TABLE', DB_PREFIX . 'blogs');
...
class Blogs_model extends \Model 
{
	...
	protected $ci;
	protected $moderation_type = 'blogs';
	...
	function get_blogs_count($params = array(), $filter_object_ids = null) 
	{
	...
	function get_blogs_list($page = null, $items_on_page = null, $order_by = null, $params = array(), $filter_object_ids = null) 
	{
		$this->ci->db->from(BLOGS_TABLE);
		if (isset($params["where"]) && is_array($params["where"]) && count($params["where"])) {
			foreach ($params["where"] as $field => $value) {
	...
	public function format_blog($data) 
	{
		...
		foreach ($data as $key=>$blog){
			...
			if ($blog['is_hidden']==1) $data[$key]['type'] = l('private', 'blogs'); else $data[$key]['type'] = l('public', 'blogs');
	...
	public function validate_blog($data = array()) 
	{
        ...
        if (isset($data['title'])) {
            ...
            if (empty($return['data']['title'])) {
                $return['errors'][] = l('error_blog_title_invalid', 'blogs');
            }
		}
        ...
        if (isset($data['description'])) {
            ...
            if (empty($return['data']['description'])) {
                $return['errors'][] = l('error_blog_short_description_invalid', 'blogs');
            }
        }
	...
	public function validate_post($data = array()) 
	{
        ...
        if (isset($data['title'])) {
            $return['data']['title'] = trim(strip_tags($data['title']));
            if (empty($return['data']['title'])) {
                $return['errors'][] = l('error_post_title_invalid', 'blogs');
            }
		}
        ...
        if (isset($data['body'])) {
            ...
            if (empty($return['data']['body'])) {
                $return['errors'][] = l('error_post_body_invalid', 'blogs');
            }
        }
	...
	private function _delete_wall_event($params) 
	{
		$this->ci->load->helper('wall_events_default');
	...
	public function _format_wall_events($events) 
	{
		...
		if ($posts_ids) {
			...
			foreach ($posts as $key=>$post){
				$posts[$key]['blog'] = $this->get_blog_by_id($post['blog_id']);
			}
		}
		foreach ($events as $key => $e) {
			$this->ci->template_lite->assign('posts', $posts);
			$this->ci->template_lite->assign('event', $e);
			$e['html'] = $this->ci->template_lite->fetch('wall_events_posts', null, 'blogs');
	...
	public function get_blog_post_by_day($date, $user_id = 0) 
	{
		...
		foreach ($results as $item){
			...
			$blog_post[$i]["post_link"] = site_url()."blogs/view_post/".$item['id'];
	...
	function get_sitemap_urls() 
	{
		...
		$block[] = array(
			"name" => l('header_my_blog', 'blogs'),
			"link" => rewrite_link('blogs', 'index'),
			"clickable" => ($auth=="user"),
			"items" => array(
				array(
					"name" => l('header_blog_calendar', 'blogs'),
					"link" => rewrite_link('blogs', 'calendar'),
					"clickable" => ($auth == "user"),
				),
				array(
					"name" => l('header_blog_friends', 'blogs'),
					"link" => rewrite_link('blogs', 'friends'),
					"clickable" => ($auth == "user"),
				),
				array(
					"name" => l('header_blog_categories', 'blogs'),
					"link" => rewrite_link('blogs', 'categories'),
					"clickable" => ($auth == "user"),
				),
			)
		);
	...
	{
		$return[] = array('link' => 'blogs/index', 'name' => l('header_my_blog', 'blogs'));
		$return[] = array('link' => 'blogs/calendar', 'name' => l('header_blog_calendar', 'blogs'));
		$return[] = array('link' => 'blogs/friends', 'name' => l('header_blog_friends', 'blogs'));
		$return[] = array('link' => 'blogs/categories', 'name' => l('header_blog_categories', 'blogs'));
		$return[] = array('link' => 'blogs/view_category', 'name' => l('header_my_blog', 'blogs'));
		return $return;
	}
