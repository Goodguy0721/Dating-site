function media(optionArr) {
    ...
    this.uninit = function () {
        ...
        $("[data-media=" + _self.properties.addPhotoID + "]").unbind();
        $("[data-media=" + _self.properties.addVideoID + "]").unbind();
        $("[data-media=" + _self.properties.addAudioID + "]").unbind();
    ...
    this.init_item_block_info = function () {
        $('#' + _self.properties.galleryContentDiv)
            ...
            if (album_id) {
                _self.properties.albumId = album_id;
                _self.load_content(true);
    ...
    this.init_links = function () {
        $("[data-media=" + _self.properties.addPhotoID + "]").unbind('click').bind('click', function () {
            _self.display_upload_photos_form();
        });
        $("[data-media=" + _self.properties.addVideoID + "]").unbind('click').bind('click', function () {
            _self.display_upload_video_form();
        });
        $("[data-media=" + _self.properties.addAudioID + "]").unbind('click').bind('click', function () {
    ...
    this.get_post_data = function () {
        ...
        if ($('#' + _self.properties.sorterId).size()) {
            ...
            data.direction = $('#' + _self.properties.sorterId).find('[data-role="sorter-dir"]').hasClass('fa-arrow-up') ? 'asc' : 'desc';
        }
        return data;
    };
    ...
    this.refresh_recent_photos = function () {
        $.ajax({
            ...
            type: 'POST',
            data: {count: 16, upload_gid: 'photo', id_user: _self.properties.idUser},
            dataType: "JSON",
            cache: false,
            beforeSend: function() {
                return preCheckAccess(_self.properties.recentMediaUrl);
            },
            success: function (data) {
                if (data.errors) {
                    _self.properties.errorObj.show_error_block(data.errors, 'error');
                } else {
                    $(_self.properties.refreshRecentPhotosDiv).html(data.html);
                }
            }
        });
    };
