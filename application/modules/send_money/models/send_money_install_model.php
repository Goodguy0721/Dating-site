 ...
 * @author 		Pilot Group Ltd <http://www.pilotgroup.net/>
 */
class Send_money_install_model extends \Model
{
    /**
    ...
    protected $menu = array(
                ...
                "other_items" => array(
                    ...
                    "items"  => array(
                        "add_ons_items" => array(
                            "action" => "none",
                            "items"  => array(
                                "send_money_menu_item" => array("action" => "create", "link" => "admin/send_money/index", "status" => 1, "sorter" => 1),
                            ),
                        ),
                    ),
                ),
        ...
        'admin_send_money_menu' => array(
            'action' => 'create',
            'name'   => 'Admin mode - Add-ons - Send money menu',
            'items'  => array(
                'send_money_settings_item' => array('action' => 'create', 'link' => 'admin/send_money/settings', 'status' => 1, "sorter" => 10),
                'send_money_view_item'     => array('action' => 'create', 'link' => 'admin/send_money/view', 'status' => 1, "sorter" => 20),
            ),
        ),
    );
    ...
    protected $moderators = array(
        array("module" => "send_money", "method" => "index", "is_default" => 0),
    );
    ...
    protected $payment_types = array(
        array(
            'gid'             => 'send_money',
            'callback_module' => 'send_money',
            'callback_model'  => 'Send_money_model',
            'callback_method' => 'paymentSendMoneyStatus',
        ),
    );
    ...
    protected $notifications = array(
        "templates" => array(
            array("gid" => "send_money_msg", "name" => "Send money message", "vars" => array("money", "approve", "decline", "id", "sender"), "content_type" => "html"),
        ),
        "notifications" => array(
            array('gid' => 'send_money_msg', 'send_type' => 'simple'),
        ),
    );
    ...
    /**
     * Install menu data of Send_money
     *
     * @return void
     */
    ...
    /**
     * Import menu languages of send_money
     *
    ...
    public function install_menu_lang_update($langs_ids = null)
    {
        ...
        $langs_file = $this->CI->Install_model->language_file_read("send_money", "menu", $langs_ids);

        if (!$langs_file) {
            log_message("info", "Empty menu langs data (send_money)");
    ...
    /**
     * Export menu languages of send_money
     *
     * @param array $langs_ids languages identifiers
     *
     * @return array
    ...
    {
        $langs_file = $this->CI->Install_model->language_file_read('send_money', 'moderators', $langs_ids);

        // install moderators permissions
        $this->CI->load->model('Moderators_model');
        $params['where']['module'] = 'send_money';
    ...
    public function install_payments_lang_update($langs_ids = null)
    {
        ...
        $langs_file = $this->CI->Install_model->language_file_read('send_money', 'payments', $langs_ids);
        if (!$langs_file) {
            log_message('info', 'Empty payments langs data (send_money)');
    ...
    public function install_notifications_lang_update($langs_ids = null)
    {
        ...
        $langs_file = $this->CI->Install_model->language_file_read("send_money", "notifications", $langs_ids);
        if (!$langs_file) {
            log_message("info", "Empty notifications langs data (send_money)");
    ...
    {
        $langs_file = $this->CI->Install_model->language_file_read("send_money", "arbitrary", $langs_ids);
        if (!$langs_file) {
            log_message("info", "Empty send_money arbitrary langs data");
    ...
    {
        $this->CI->pg_seo->delete_seo_module('send_money');
