function formFields(optionArr){
	...
	this.build_field = function(gid, type, name, section_gid){
		...
		name = name !== '' ? name : '&nbsp;';
		$('#'+parent_id).append('<li id="field_'+gid+'"><div id="field_'+gid+'_actions" class="pull-right"></div><span class="field_type">'+type+': '+name+'</span></li>');
		$('#field_'+gid).addClass('type_'+type);

		if(type == 'select' || type == 'text' || type == 'range'){
			$('#field_'+gid+'_actions').append(' <a href="javascript:;" class="edit-btn">' + _self.properties.textEdit + '</a>');
			$('#field_'+gid+'_actions .edit-btn').bind('click', function(){
				_self.field_settings_form(gid);
			});
		}

		$('#field_'+gid+'_actions').append(' <a href="javascript:;" class="delete-btn">' + _self.properties.textDelete + '</a>');
		$('#field_'+gid+'_actions .delete-btn').bind('click', function(){
	...
	this.build_section = function(gid, name, fields){
		$('#form_root').append('<li id="section_'+gid+'"><div id="section_'+gid+'_actions" class="pull-right"></div><span class="field_type">section: '+name+'</span></li>');

		$('#section_'+gid+'_actions').append(' <a href="javascript:;" class="add-btn">' + _self.properties.textAdd + '</a>');
		$('#section_'+gid+'_actions .add-btn').bind('click', function(){
			_self.add_field_section_form(gid);
		});

		$('#section_'+gid+'_actions').append(' <a href="javascript:;" class="edit-btn">' + _self.properties.textEdit + '</a>');
		$('#section_'+gid+'_actions .edit-btn').bind('click', function(){
			_self.add_section_form(gid);
		});

		$('#section_'+gid+'_actions').append(' <a href="javascript:;" class="delete-btn">' + _self.properties.textDelete + '</a>');
		$('#section_'+gid+'_actions .delete-btn').bind('click', function(){
			_self.delete_section(gid);
		});

		$('#section_'+gid).append('<ul id="sfields_'+gid+'" class="sortable to_do"></ul>');
