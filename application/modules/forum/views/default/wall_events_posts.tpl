	...
	<div>
		<div class="ptb5">
			{if $event.event_type_gid eq 'blog_post_created'}
				<a href="{$posts[$id_post].user.link}">{$posts[$id_post].user.output_name}</a> {l i='wall_created_post' gid='blogs'} <a href="{$site_url}blogs/view_post/{$id_post}">{l i='post' gid='blogs'}</a>
			{/if}
		</div>
	</div>
{/foreach}
