...
//				}
    ...
    _p.renderContact = function (contact, age_text, status_lang, method) {
        ...
        html += '</div>' +
                ...
                '<div class="im-contact__more-info hide">' +
        ...
        if (method === 'append') {
            ...
            if (!_p.contact_list_obj.find('#im_contact_list_contact_' + contact.id).size()) {
    ...
    _p.renderMessages = function (id_contact, messages, type, make_assoc) {
        ...
        var prev_height = _p.msg_chat_obj.get(0).scrollHeight;
        switch (type) {
            case 'replace':
    ...
    _p.initDomObjects = function () {
        ...
        _p.contact_list_obj.off('click', '.im-contact').on('click', '.im-contact', function () {
            ...
            if ($(_self.properties.imMobileBlock).is(':hidden')) {
                ...
                var top = _p.panel_obj.position().top + $(this).position().top + ($(this).outerHeight() / 2);
                var right = _p.contact_list_obj.width();
                _p.info_popup_obj.css({top: top + 'px', right: right + 'px'});
