 ...
 * */
class Oovoochat extends Chat_abstract
{
    ...
    public function get_last_chats_count($params = array(), $filter_object_ids = null)
    {
        ...
        $result = $this->DB->count_all_results(OOVOOCHATS_TABLE);
    ...
    {
        if (is_null($id_chat)) {
            $this->DB->insert(OOVOOCHATS_TABLE, $attrs);
            $id_chat = $this->DB->insert_id();
        } else {
            $this->DB->where('id', $id_chat);
            $this->DB->update(OOVOOCHATS_TABLE, $attrs);
        }
    ...
    {
        $this->DB->where('id', $id_chat);
        $this->DB->delete(OOVOOCHATS_TABLE);
    ...
    public function canceled_chats()
    {
        ...
        $this->DB->update(OOVOOCHATS_TABLE, array('status' => 'canceled'));
    ...
    public function write_off_for_chat($id_user, $price, $message)
    {
    ...
    public function setNotified($chats_ids)
    {
        ...
        $this->DB->update(OOVOOCHATS_TABLE);
    }
}
