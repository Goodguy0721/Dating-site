...
class CI_Loader
{
    ...
    public function model($model, $name = '', $db_conn = false, $create_object = true, $return_status = false)
    {
        ...
        if ($path) {
            ...
            $module = strtolower(substr($model, 0, strpos($model, '_model')));
            $model_path = MODULEPATH . $path . $module . '/models/' . $path . $model . EXT;
        }

        if (!file_exists($model_path)) {
            $model_path = APPPATH . 'models/' . $path . $model . EXT;
        }
        ...
        if ($create_object) {
            ...
            if (!class_exists($model, false)) {
                // Try with namespace
                $model_ns = $this->pathToNamespace($model_path);
                $CI->{$name} = new $model_ns();
            } else {
                $CI->{$name} = new $model();
            }
            /**
             * @todo romeve this from method: $CI->{$name}->_assign_libraries();
             */
    ...
    private function pathToNamespace($path)
    {
        $full_path = str_replace(
            array(SITE_PHYSICAL_PATH . 'application', '/', EXT), array('', ' ', ''), $path);

        return NS_PREFIX . str_replace(' ', '\\', ucwords(ltrim($full_path)));
    }
     ...
     * @return void
     */
    public function helper($helpers = array(), $module = null)
    {
        if (!is_array($helpers)) {
