...
class CI_Image_lib
{
    ...
    public $library_path   = PATH_TO_IMAGE_MAGIC;
    ...
    public $wm_text_length     = 0; // Watermark text length
    ...
    {
        $props = array('source_folder', 'dest_folder', 'source_image', 'full_src_path', 'full_dst_path', 'new_image', 'image_type', 'size_str', 'quality', 'orig_width', 'orig_height', 'rotation_angle', 'x_axis', 'y_axis', 'create_fnc', 'copy_fnc', 'wm_overlay_path', 'wm_use_truetype', 'dynamic_output', 'wm_font_size', 'wm_text', 'wm_vrt_alignment', 'wm_hor_alignment', 'wm_padding', 'wm_hor_offset', 'wm_vrt_offset', 'wm_font_color', 'wm_use_drop_shadow', 'wm_shadow_color', 'wm_shadow_distance', 'wm_opacity', 'grayscale', 'background', 'bgcolor', 'bgwidth', 'bgheight');
    ...
    public function rotate()
    {
        ...
        $degs = array(90, 180, 270, 'vrt', 'hor', -10);
    ...
    public function image_process_gd($action = 'resize')
    {
        ...
        if ($action == 'crop') {
            ...
            if ($this->orig_width < $this->width || $this->orig_height < $this->height) {
                ...
                $width  = max($this->width, $this->orig_width);
                $height = max($this->height, $this->orig_height);

                $this->x_axis += ($width - $this->orig_width) / 2;
                $this->y_axis += ($height - $this->orig_height) / 2;

                $bgcolor  = array('red' => 255, 'green' => 255, 'blue' => 255);
                $temp_img = $this->image_background_gd($src_img, $width, $height, $bgcolor);
        ...
        if (isset($this->grayscale) && $this->grayscale) {
            $dst_img = $this->image_grayscale_gd($dst_img, $this->width, $this->height);
        }

        if (isset($this->background) && $this->background) {
            if (!isset($this->bgcolor)) {
                $this->bgcolor = array('red' => 255, 'green' => 255, 'blue' => 255);
            }
            ...
            if (empty($this->bgwidth)) {
                $this->bgwidth = $this->width;
            }

            if (empty($this->bgheight)) {
                $this->bgheight = $this->height;
            }

            $temp_img = $this->image_background_gd($dst_img, $this->bgwidth, $this->bgheight, $this->bgcolor);
    ...
    public function image_grayscale_gd($src_img, $width, $height)
    {
        $img = imageCreate($width, $height);
        for ($c = 0; $c < 256; ++$c) {
            ImageColorAllocate($img, $c, $c, $c);
        }
        ImageCopyMerge($img, $src_img, 0, 0, 0, 0, $width, $height, 100);

        return $img;
    }

    public function image_background_gd($src_img, $width, $height, $color_array = array('red' => 255, 'green' => 255, 'blue' => 255))
    {
        $swidth  = imagesx($src_img);
        $sheight = imagesy($src_img);

        if ($swidth > $width || $sheight > $height) {
            return $src_img;
        }

        $x = intval(($width - $swidth) / 2);
        $y = intval(($height - $sheight) / 2);

        $img        = imagecreatetruecolor($width, $height);
        $background = imagecolorallocate($img, $color_array['red'], $color_array['green'], $color_array['blue']);
        imagefilledrectangle($img, 0, 0, $width, $height, $background);
        imagecopyresampled($img, $src_img, $x, $y, 0, 0, $swidth, $sheight, $swidth, $sheight);
    ...
    public function overlay_watermark()
    {
        ...
        if ($this->wm_opacity == 100) {
            imagecopy($src_img, $wm_img, $x_axis, $y_axis, 0, 0, $wm_width, $wm_height);
        } else {
            $temp_img = imagecreatetruecolor($wm_width, $wm_height);
            imagefilledrectangle($temp_img, 0, 0, $wm_width - 1, $wm_height - 1, imagecolorallocate($temp_img, 255, 255, 255));
            imagecopy($temp_img, $src_img, 0, 0, $x_axis, $y_axis, $wm_width, $wm_height);
            imagecopy($temp_img, $wm_img, 0, 0, 0, 0, $wm_width, $wm_height);
            imagedestroy($wm_img);
            imagecopymerge($src_img, $temp_img, $x_axis, $y_axis, 0, 0, $wm_width, $wm_height, $this->wm_opacity);
    ...
    public function text_watermark()
    {
        ...
        $this->wm_text_length = mb_strlen($this->wm_text, 'UTF-8');
        ...
        if ($this->wm_use_truetype == true) {
            ...
            $fontrect = imagettfbbox($this->wm_font_size, 0, $this->wm_font_path, $this->wm_text);
            $fontwidth = ($fontrect[2] + 2) / $this->wm_text_length;
        ...
        switch ($this->wm_hor_alignment) {
            ...
            case "R":
                if ($this->wm_use_drop_shadow) {
                    $x_shad += ($this->orig_width - $fontwidth * $this->wm_text_length);
                }
                $x_axis += ($this->orig_width - $fontwidth * $this->wm_text_length);
                break;
            case "C":
                if ($this->wm_use_drop_shadow) {
                    $x_shad += floor(($this->orig_width - $fontwidth * $this->wm_text_length) / 2);
                }
                $x_axis += floor(($this->orig_width - $fontwidth * $this->wm_text_length) / 2);
                break;
        }
    ...
    public function image_create_gd($path = '', $image_type = '')
    {
        ...
        switch ($image_type) {
            ...
            case 3 :
                ...
                $imgPng = imagecreatefrompng($path);
    ...
    public function image_save_gd($resource)
    {
        switch ($this->image_type) {
            ...
            case 2 :
                ...
                @imagejpeg($resource, $this->full_dst_path ? $this->full_dst_path : null, $this->quality);
                break;
            case 3 :
