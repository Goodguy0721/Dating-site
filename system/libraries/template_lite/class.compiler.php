...
//		$this->_dvar_regexp = '\$[a-zA-Z0-9_]{1,}(?:' . $this->_var_bracket_regexp . ')*(?:' . $this->_var_bracket_regexp . ')*';
        //$this->_dvar_regexp = '\$[a-zA-Z0-9_]{1,}(?:' . $this->_var_bracket_regexp . ')*(?:\.\$?\w+|->\$?\w+(?:' . $this->_var_bracket_regexp . ')*)*';
...
//		$this->_func_regexp = '[a-zA-Z_]\w*';
    ...
    public function _compile_file($file_contents)
    {
        ...
        // remove php blocks
        preg_match_all("!{$ldq}\s*php\s*{$rdq}(.*?){$ldq}\s*/php\s*{$rdq}!s", $file_contents, $_match1); // old style
        preg_match_all("!{$ldq}\s*php\s*(.*?){$rdq}!s", $file_contents, $_match2); // new style {php echo 1;}

        $_match = $_match1[1] + $_match2[1];

        $this->_php_blocks = $_match;
        ...
        foreach ($this->_require_stack as $key => $value) {
            $compiled_text = "<?php\nrequire_once('" . $this->_get_plugin_dir($key) . $key . "');\n"
                    . '$this->register_' . $value[0] . '("' . $value[1] . '", "' . $value[2] . "\"); ?>" . $compiled_text;
        }
    ...
    public function _parse_function($function, $modifiers, $arguments)
    {
        switch ($function) {
            ...
            case 'foreach':
                ...
                $_result = '<?php if (is_array(' . $_args['from'] . ') and count((array)' . $_args['from'] . ')): foreach ((array)' . $_args['from'] . ' as ' . $_args['key'] . '$this->_vars[\'' . $_args['value'] . '\']): ?>';
            ...
            case 'depends':
                ...
                $CI = &get_instance();
                $_result = '';
                if (!$CI->pg_module->is_module_active($_args['module'])) {
                    $_result = '<?php /*';
                    $this->_comment_start = true;
                }
                if (!$this->_comment_start) {
                    ...
                    for ($i = 1;;++$i) {
                        ...
                        if (empty($_args[$key])) {
                            ...
                            if (!$CI->pg_module->is_module_active($_args[$key])) {
                                $_result = '<?php /*';
                                $this->_comment_start = true;
                                break;
                            }
    ...
    public function _compile_config($variable)
    {
...
//			preg_match_all('/(?:[^\\\]' . $this->_dvar_regexp . '[^\\\])/', $variable, $_expand);
    ...
    public function _compile_variable($variable)
    {
        ...
        preg_match_all('!(?:^\w+)|(?:' . $this->_var_bracket_regexp . ')|\.\$?\w+|\->\$?\w+|\S+!', $variable, $_match);
        $variable = $_match[0];
