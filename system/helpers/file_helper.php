 ...
 * Download File
 *
 * Downloads the file specfied in the $src_file and saves it as a $dst_file.
 *
 * @param string $src_file
 * @param string $dst_file
 * @param boolean $fast_way
 *
 * @return boolean
 */
if (!function_exists('download_file')) {
    function download_file($src_file, $dst_file, $fast_way = false, $mode = 0644)
    {
        if ($fast_way) {
            $result = file_put_contents($dst_file, file_get_contents($src_file));
            chmod($dst_file, $mode);

            return $result > 1;
        } else {
            $srcHandle = fopen($src_file, 'rb');
            if (!$srcHandle) {
                return false;
            }
            $dstHandle = fopen($dst_file, 'w');
            if (!$dstHandle) {
                fclose($srcHandle);
            ...
            while (!feof($srcHandle)) {
                $content = fread($srcHandle, 8192);
                fwrite($dstHandle, $content);
            }
            fclose($srcHandle);
            fclose($dstHandle);
            chmod($dst_file, $mode);
